function state_arm_idle(argument0, argument1) {
    ...
    switch (argument1) {
        ...
        case SM_EVENT.create:
            // No initialization required here, but consider documenting this state.
            break;
        ...
        case SM_EVENT.update:
            if (global.game_click) {
                switch (current_tool) {
                    ...
                    case TOOLS.glass:
                        // Try to find a creature or treasure instance
                        var creature = instance_position_tool(par_creature);
                        if (!creature) creature = instance_position_tool(par_treasure);
                        if (!creature) creature = instance_nearest(x + target_x, y + target_y, par_creature);

                        if (creature && (instance_position_tool(creature) || 
                            point_distance(x + target_x, y + target_y, creature.x, creature.y) < examination_small_range + creature.exam_range)) {
                    ...
                    case TOOLS.claw:
                        ...
                        if (creature) {
                            ...
                            if (creature.grabbable) {
                                ...
                                if (creature.object_index == obj_sun) {
                                    achievement_earn("Hot, Hot, Hot");
                                }
                    ...
                    case TOOLS.poker:
                        // Trigger user event on the targeted creature
                        var creature = instance_position_tool(par_creature);
                        if (creature) with (creature) event_user(0);
                        break;
                }
            }
            break;
